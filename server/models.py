...
class Wallet(Base):
    ...
    balances = relationship("WalletBalance", back_populates="wallet")  # Add relationship to WalletBalance

class WalletBalance(Base):
    __tablename__ = "wallet_balance"
    
    wid = Column(Integer, ForeignKey("wallet.wid"), primary_key=True)
    cid = Column(String, ForeignKey("cryptocurrency.cid"), primary_key=True)
    balance = Column(Numeric(20, 10), default=0)  # Cryptocurrency balance in this wallet
...
Index('idx_transaction_uid_cid_time', Transaction.uid, Transaction.cid, Transaction.time_transaction)
Index('idx_wallet_uid_address', Wallet.uid, Wallet.address)
Index('idx_wallet_balance_wid_cid', WalletBalance.wid, WalletBalance.cid)
